...
module.exports = {
	...
	async execute(interaction) {
		...
		if(!interaction.member.roles.cache.has(e => e.name.toLowerCase().includes('Head Moderator') || e.name.toLowerCase().includes('Admin') || e.name.toLowerCase().includes('Manager') || e.name.toLowerCase().includes('Bot Dev')) && oldPunishment.author !== interaction.user.id)return interaction.reply({ content: 'Only a head moderator or above can edit punishments made by other moderators.', ephemeral: true});
